...
class CreateUsersTable extends Migration {
	...
	public function up()
	{
		Schema::create('users', function(Blueprint $table)
		{
			...
			$table->string('ip', 15)->nullable()->index('ip');
			...
			$table->string('alias', 12)->nullable()->index('alias');
			$table->string('email2', 120)->nullable();
			$table->char('voiptovoip', 1)->nullable();
			...
			$table->char('usa_4003', 1)->default('n');
			$table->float('venda_4003', 7, 4)->default(0.0000);
			$table->float('venda_0800_fixo', 7, 4)->default(0.0000);
			$table->float('venda_0800_celular', 7, 4)->default(0.0000);
			$table->char('socobrar_4003_secompletar', 1)->default('n');
			$table->char('cobrar_4003_emumatarifaso', 1)->default('n');
			$table->char('tipo_lig_permitidas_4003', 1)->default('t');
			$table->string('tp_tarifacao_4003', 4)->default('6006');
			$table->char('usa_sms', 1)->default('n');
			$table->float('venda_sms', 7, 4)->default(0.2500);
			...
			$table->string('portabilidade', 1)->default('n');
			$table->text('codigo_rn_liberado', 16777215);
			$table->integer('tipo_restricao_ligacao')->default(0)->comment('determina o tipo de restricao para ligacoes, serve como controle da tabela FonesRestricao. Valores possiveis:0=sem restricoes, 1=lista branca(so pode ligar para os numeros da tabela), 2=lista negra (nao pode ligar para os numeros da tabela)');
			$table->string('usr_telegram_token', 100)->nullable();
			$table->char('usr_mostra_revenda', 1)->nullable();
			$table->string('acessoippainel', 15)->nullable();
		});
	}
